...
top05 <- all.markers %>% group_by(cluster) %>% filter(p_val_adj<0.05)
### create a single file
#markers <- top05 %>% filter(gene%in%vector)
#markers2 <- all.markers %>% filter(gene%in%vector)
...
marker.excel.pages <- list('clust0' = top05 %>% filter(cluster==0),
                           ...
                           "clust30"= top05 %>% filter(cluster==30),
                           "clust31"= top05 %>% filter(cluster==31),
                           "clust32"= top05 %>% filter(cluster==32))

top05 <- top05[order(top05$avg_log2FC,decreasing = T),]
write_xlsx(top05, "results/000_topgenes_cluster.xlsx")
#write_xlsx(markers, "results/000_gene_markers.xlsx")
#write_xlsx(marker.excel.pages, "results/000_Cluster_markers_p05_aggregate_ver.xlsx")
...
######### New custom clustering
levels(data$seurat_clusters) #32
data <- subset(data, idents = 32, invert = TRUE)
named.clusters <- RenameIdents(data,
                               `0` = "CD4+ T cells",
                               `1` = "Monocytes",
                               `2` = "Macrophages",
...
data<-named.clusters
png("plots/006_CvsT.png",w=2500,h=2500,res = 300)
DimPlot(object = data,pt.size=1,reduction='tsne',group.by='Treat',label=F,cols=c("grey","red")) 
dev.off()

png("plots/006b_CvsT.png",w=5000,h=2500,res = 300)
DimPlot(object = data,pt.size=1,reduction='tsne',group.by='Treat',split.by='Treat',label=F,cols=c("grey","red")) 
dev.off()
...
library(ggplot2)
png("plots/006_Dotplot.png",h=2000,w=2500,res=300)
DotPlot(
  object = data, features = genes
) + scale_colour_gradient2(low = "blue", mid = "white", high = "red")+
  coord_flip() +xlab("")+ylab("")+ 
...
# Absolute frequency barplot
png("plots/006_stacked_barplot.png",w=3500,h=2500,res=300)
par(mar=c(5,4,4,20),xpd=T)
barplot(my_table2, main = "Relative frequency (%)",
        col = color_list,border = "white" )
...
>>>>>>> 7bed995573d08197c717e7a149d70ae6181d8efa
